...
class ReGSAT(nn.Module):
    ...
    def multipliers_update(self, loss_dict):
        self.info_loss_multiplier += self.dual_lr * (loss_dict['info']-self.info_loss_tol-(1/self.resilient_alpha)*self.info_loss_multiplier) 
        self.prox_loss_multiplier += self.dual_lr * (loss_dict['prox']-self.prox_loss_tol-(1/self.resilient_alpha)*self.prox_loss_multiplier)
    ...
    def train_one_batch(self, data, epoch):
        ...
        loss.backward()
        self.optimizer.step()
        # (IH) multiplier update
        self.multipliers_update(loss_dict)
        return att.data.cpu().reshape(-1), loss_dict, clf_logits.data.cpu()
